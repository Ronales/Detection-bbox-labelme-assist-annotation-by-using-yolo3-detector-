...
class Multiply(meta.Augmenter):
    ...
    def _augment_images(self, images, random_state, parents, hooks):
        ...
        mul_samples = self.mul.draw_samples(
            (nb_images, nb_channels_max), random_state=rss[1])
        #modify some change in Multiply  11.16
        #!!!!!!!!!!!!!!!!!!!!!!!
        from random import shuffle   #[1,1,1]  is original para
        mul_samples_mod=[[1,1,2.5],[2.5,2.5,2.5],[1,1,1],[0.5,0.5,1.5],[1,0.5,2.5]]
        shuffle(mul_samples_mod)
        mul_samples_mod=np.array(mul_samples_mod*nb_images)


        gen = enumerate(zip(images, mul_samples_mod, per_channel_samples))
